...
const allowedExt = ['.js', '.map', '.css', '.ico', '.png', '.jpg', '.svg', '.woff2', '.woff', '.ttf'];
...
export class FrontendMiddleware implements NestMiddleware {
    use(req: Request, res: Response, next: any) {
        ...
        if (/^\/api\/(\S+)?$/.test(url)) {
             ...
             */
            res.sendFile(resolve(join('..', 'client', 'dist', 'client', `${url}`)));
        } else {
            res.sendFile(resolve(join('..', 'client', 'dist', 'client', 'index.html')));
        }
    }
}
